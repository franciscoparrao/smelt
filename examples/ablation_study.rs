...
// ── Simplified gradient boosting training loop ─────────────────────────

fn sigmoid(x: f64) -> f64 {
    1.0 / (1.0 + (-x).exp())
}

/// Variant 1: row-major u16, scan all nodes, no subtraction
fn train_v1_rowmajor_u16(features: &Array2<f64>, target: &[usize], n_trees: usize) -> f64 {
    ...
    for _ in 0..n_trees {
        ...
        // Build histogram per feature — ROW-MAJOR scan
        ...
        for feat in 0..nf {
            ...
            for i in 0..ns {
                let b = bins.get_bin(i, feat) as usize; // ROW-MAJOR access
                bin_g[b] += grads[i];
                bin_h[b] += hess[i];
            }
            let total_g: f64 = bin_g.iter().sum();
            let total_h: f64 = bin_h.iter().sum();
            ...
            for b in 0..nb.saturating_sub(1) {
                ...
                let gain = gl * gl / (hl + 1.0) + gr * gr / (hr + 1.0)
        ...
        // Simple split: everyone goes left or right (simplified, not a real tree)
        for i in 0..ns {
            let b = bins.get_bin(i, best_feat) as usize;
            let leaf_g: f64 = if b <= best_bin {
                grads.iter().take(ns / 2).sum()
            } else {
                grads.iter().skip(ns / 2).sum()
            };
            let leaf_h: f64 = if b <= best_bin {
                hess.iter().take(ns / 2).sum()
            } else {
                hess.iter().skip(ns / 2).sum()
            };
            let w = -leaf_g / (leaf_h + 1.0);
            fv[i] += lr * w;
        }
    }
    fv.iter().sum::<f64>() / ns as f64
}

/// Variant 2: column-major u16, scan all nodes, no subtraction
fn train_v2_colmajor_u16(features: &Array2<f64>, target: &[usize], n_trees: usize) -> f64 {
    let ns = features.nrows();
    let nf = features.ncols();
    ...
    for _ in 0..n_trees {
        ...
        for feat in 0..nf {
            ...
            let mut bin_h = vec![0.0; nb];
            for i in 0..ns {
                let b = bins.get_bin(feat, i) as usize; // COLUMN-MAJOR access
                bin_g[b] += grads[i];
                bin_h[b] += hess[i];
            }
            let total_g: f64 = bin_g.iter().sum();
        ...
        for i in 0..ns {
            let b = bins.get_bin(best_feat, i) as usize;
            let leaf_g: f64 = if b <= best_bin {
                grads.iter().take(ns / 2).sum()
            } else {
                grads.iter().skip(ns / 2).sum()
