...
fn main() {
    ...
    println!("Peak RSS after XGBoost Rust (N=10K): {} MB", get_peak_rss_kb() / 1024);

    let mut cb = CatBoost::new().with_n_estimators(100).with_depth(6);
    let _ = cb.train_classif(&task).unwrap();
    println!("Peak RSS after CatBoost Rust (N=10K): {} MB", get_peak_rss_kb() / 1024);
}
