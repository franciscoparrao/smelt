//! Performance benchmark: smelt-ml CatBoost.
//! Run with: cargo test --test catboost_perf --release -- --nocapture --test-threads=1
...
fn load_dataset(prefix: &str, n: usize) -> (Array2<f64>, Vec<f64>) {
    ...
    let target: Vec<f64> = y_str.lines().filter(|l| !l.is_empty())
...
fn catboost_performance_regress() {
    ...
    for &n in &sizes {
        ...
        let t0 = std::time::Instant::now();
        let _model = cb.train_regress(&task).unwrap();
        let elapsed = t0.elapsed().as_secs_f64() * 1000.0;
