...
impl HistPool {
    pub fn new(max_depth: usize, n_col: usize, max_bins: usize) -> Self {
        ...
        Self { levels, max_bins, feat_stride }
    }
    ...
    pub fn find_best(&self, depth: usize, col_indices: &[usize], bins: &HistBins,
                     min_cw: f64, lambda: f64, _alpha: f64, gamma: f64)
        -> Option<(usize, f64, f64, bool, usize)>
    {
        ...
        let results: Vec<Option<(usize, f64, f64, bool, usize)>> =
            col_indices.par_iter().enumerate().map(|(fi, &feat)| {
            let off = fi * fs;
            ...
            for b in 0..nb.saturating_sub(1) {
                gl += bin_g[b]; hl += bin_h[b];
                let (gr, hr) = (total_g - gl, total_h - hl);
                if hl < min_cw || hr < min_cw { continue; }
                let g = sg(gl, hl, gr, hr);
                if g > best_gain { best_gain = g; best_bin = b; best_nan_left = false; }
            }
            if nan_h > 0.0 {
                ...
                for b in 0..nb.saturating_sub(1) {
                    ...
                    if hl < min_cw || hr < min_cw { continue; }
                    let g = sg(gl, hl, gr, hr);
                    if g > best_gain { best_gain = g; best_bin = b; best_nan_left = true; }
                }
            }

            if best_gain > 0.0 {
                Some((feat, bins.bin_threshold(feat, best_bin), best_gain, best_nan_left, best_bin))
            } else {
                None
            }
        }).collect();
